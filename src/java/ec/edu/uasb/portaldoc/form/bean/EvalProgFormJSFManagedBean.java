 ...
 * @author victor.barba
 */
@ManagedBean(name = "evalProgFormBean")
@ViewScoped
public class EvalProgFormJSFManagedBean implements Serializable {
     ...
     */
    public EvalProgFormJSFManagedBean() {
    ...
    public void init() {
        ...
        catalogo = encuestaRealizadaFacade.encuestasDisponibles(codCordinador, "EVAL_PROGRAMA");
...
//                            System.out.println(" dynaFormControl.getKey()" + ((Pregunta) ff.getKey()).getPreguntaPK().getCodigoPregunta() + " dynaFormControl.getKey()" + dynaFormControl.getKey() + " dynaFormControl.getRow() " + dynaFormControl.getRow() + " ff.getValue() " + ff.getValue() + "  " + dynaFormControl.getType());
        ...
        if (ok) {
            msgError = encuestaRealizadaFacade.createEncuesta(codCordinador, encuEvalDocente, respuestas, FacesContext.getCurrentInstance(), "Programas");
        } else {
